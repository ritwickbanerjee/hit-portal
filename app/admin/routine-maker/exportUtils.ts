...
/* ========================================================================
   ...
   ======================================================================== */
export async function exportFacultyExcel(grid: GridState, faculties: FacultyData[], sessionYear: string = "2026-27") {
    ...
    sortedFaculties.forEach((fac, facIdx) => {
        ...
        titleCell.value = 'TIME TABLE';
        titleCell.font = { name: 'Calibri', size: 14, bold: true };
        titleCell.alignment = { horizontal: 'center', vertical: 'middle' };

        sheet.mergeCells('A2:J2');
        const facCell = sheet.getCell('A2');
        facCell.value = 'Faculty: ' + dispName;
        facCell.font = { name: 'Calibri', size: 12, bold: true };
        facCell.alignment = { horizontal: 'right', vertical: 'middle' };

        // Row 3: B.Tech/M.Tech/MCA + SESSION
        sheet.mergeCells('A3:I3');
        sheet.getCell('A3').value = 'B.Tech/M.Tech/MCA';
        sheet.getCell('A3').font = { bold: true };
        sheet.getCell('J3').value = 'SESSION: ' + sessionYear;
        ...
        sheet.getCell(`A${rowIndex + 1}`).value = 'To be Effective from 20.07.2026';
        sheet.getCell(`A${rowIndex + 1}`).font = { bold: true };
        rowIndex++;

        sheet.mergeCells(`A${rowIndex + 2}:J${rowIndex + 2}`);
        sheet.getCell(`A${rowIndex + 2}`).value = 'A minimum of 75% attendance is mandatory for being eligible to sit for the End-Semester Examination';
        sheet.getCell(`A${rowIndex + 2}`).font = { bold: true };
        rowIndex++;

        sheet.mergeCells(`A${rowIndex + 3}:J${rowIndex + 3}`);
        sheet.getCell(`A${rowIndex + 3}`).value = 'Students should target 100% attendance';
        sheet.getCell(`A${rowIndex + 3}`).font = { bold: true };
        rowIndex += 2;
        ...
        const colMaxLen: number[] = new Array(11).fill(0);

        sheet.eachRow((row, rowNum) => {
            if (rowNum < 5 || rowNum > (5 + DAYS.length - 1)) return; 
