...
export default function StudentAttendance() {
    ...
    const downloadAttendancePDF = () => {
        ...
        // Calculate Per-Faculty Stats
        const facultyStats: Record<string, { total: number, attended: number, massBunks: number }> = {};

        allDates.forEach(d => {
            if (!facultyStats[d.faculty]) {
                facultyStats[d.faculty] = { total: 0, attended: 0, massBunks: 0 };
            }
            facultyStats[d.faculty].total++;
            if (d.status === 'Present') facultyStats[d.faculty].attended++;
            if (d.status === 'Mass Bunk') facultyStats[d.faculty].massBunks++;
        });

        // Prepare Summary Table Data
        const summaryBody = Object.entries(facultyStats).map(([faculty, stats]) => [
            faculty,
            stats.total,
            stats.attended,
            stats.total - stats.attended, // Absent
            stats.massBunks
        ]);
